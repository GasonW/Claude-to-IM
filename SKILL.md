...
- **CLI Session** (optional): If the user wants to bridge to an existing Claude Code CLI session (e.g., the current terminal session), ask for the `sdkSessionId`. This allows messages from IM to land in an already-running session instead of spawning a new one. Leave blank to always spawn fresh sessions.
...
If the user wants to update the CLI session binding, ask for a new `sdkSessionId` (or leave blank to clear it).
...
- CLI session not found → verify `sdkSessionId` in config.env matches a running Claude Code session, or clear it to use auto-spawned sessions
...
- CLI session binding (`sdkSessionId`) allows the bridge to attach to an existing Claude Code CLI session. If set, inbound IM messages are routed to that session instead of spawning a new one. The session ID is stable for the lifetime of the CLI process.
